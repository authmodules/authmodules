...
Run `npm run release:preflight -- <release>` before and after each release phase. It validates the committed manifest against the remote plan tag when that tag exists, resolves every package tag to a full remote commit, requires published stable GitHub Releases, checks GitHub Packages presence, and confirms that the central plan and release tags cannot diverge. It is deliberately read-only: protected tag creation, workflow dispatch, and GitHub Release creation remain explicit operator actions.
