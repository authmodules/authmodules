...
For release plans containing `scripts/prepare-package-release-evidence.js`, each package workflow checks out the package and contracts into distinct stable sibling paths, creates one exact tarball, verifies its SHA-512 integrity against the immutable plan, and publishes that same file. The `store-postgres` release check also runs against a conditional `postgres:18-alpine` service before publication. After registry integrity is confirmed, the workflow creates GitHub artifact attestations for build provenance and a CycloneDX SBOM, then retains the tarball and SBOM as workflow evidence for 90 days. Historical release plans remain rerunnable through the legacy publication path and do not claim attestations they did not create.
...
Run `npm run release:preflight -- <release>` before and after each release phase. It validates the committed manifest against the remote plan tag when that tag exists, resolves every package tag to a full remote commit, requires published stable GitHub Releases, verifies public GitHub Packages visibility and exact registry integrity, and confirms that the central plan and release tags cannot diverge. It is deliberately read-only: protected tag creation, workflow dispatch, and GitHub Release creation remain explicit operator actions.
